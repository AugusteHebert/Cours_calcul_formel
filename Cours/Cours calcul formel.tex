...
\textbf{Algorithme 12.} & Test-Fermat($n,k$)\\
           ...
           & $p\leftarrow 1$\\
           & tant que $i<k$ et $p=1$ :\\
           &\ \ \ {\rm |} choisir $\overline{a}\in (\Z/n\Z)^\times$ au hasard,\\
           & \ \ \ {\rm |} si $\overline{a}^{n-1}=\overline{1}$ : \\
           &\ \ \  \ \ \ {\rm |} $i\leftarrow i+1$\\
           & \ \ \ {\rm |} sinon $p=0$\\      
           & si $p=1$ :\\
...
\begin{definition}
Soit $n\in \N^*$ impair. Écrivons $n-1=2^s d$, avec $s\in\N^*$ et $d$ impair. Un \textbf{témoin de Miller} pour $n$ est un nombre $a\in\llbracket 1,n-1\rrbracket$
